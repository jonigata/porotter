...
MyPage.fillPosts = function(timelineId,version) {
	...
	var posts = new $("[timeline-id=\"" + timelineId + "\"]");
	var level = Std.parseInt(posts.attr("level"));
	if(!MyPage.startLoad(posts,version)) return;
	$.ajax({ url : "/foo/p/timeline", data : { timeline : timelineId, level : level}}).done(function(data) {
		console.log("timeline response receivied");
		MyPage.finishLoad(posts,function() {
			var entry = MyPage.getEntry(posts);
			var newPosts = new $(data);
...
MyPage.startLoad = function(posts,version) {
...
MyPage.finishLoad = function(posts,f) {
	var timelineId = Std.parseInt(posts.attr("timeline-id"));
...
MyPage.updateDetail = function(postId,version) {
	...
	$.ajax({ url : "/foo/p/detail", data : { post : postId, level : level}}).done(function(data) {
